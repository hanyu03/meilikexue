...
export default {
    ...
    methods: {
        getData(id){
            let _this = this;
            this.axios.get('/BeautyScience/polyv_videos/'+id)
            .then(function(respones){
                ...
                _this.plv_ObjVideo.width = respones.data.width;
                _this.plv_ObjVideo.height = respones.data.height;
                _this.plv_ObjVideo.vid = respones.data.vid;
                _this.plv_ObjVideo.session_id = data.id;
                _this.axios.post('BeautyScience/qiniu/url',{
                    res_name:data.thumb
                })
                .then(function(respones){
                    _this.pluploadImgSrc = respones.data.url
                })
                var player = polyvObject('#plv_video').videoPlayer(_this.plv_ObjVideo);
            })
        },
        imgDelete(){
            let that = this;
        ...
        Cancel(){
            this.$router.push('/media/Polyv_video')
        },
        submit(){
            let that = this;
            this.axios.patch('/BeautyScience/polyv_videos/'+this.editId,{
                polyv_video:{
            ...
            this.uploader = QiniuImg.uploader({
                ...
                init: {
                    ...
                    'FilesAdded': function (up, files) {
                        _this.axios.get('BeautyScience/uuid')
                        .then(function(response) {
                            _this.uuidImg = response.data.uuid
                        })
